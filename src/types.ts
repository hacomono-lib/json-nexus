...
export interface RefObject<R extends RefPath = RefPath> { $ref: R }

export type RefPath = `#/${string}`

export type DotPath<P extends RefPath> = P extends `#/${infer R}` ? Replace<R, '/', '.'> : never

export type Replace<S extends string, B extends string, A extends string> = S extends `${infer R1}${B}${infer R2}` ? Replace<`${R1}${A}${R2}`, B, A> : S
...
export type ReefyInternal<ORIGIN extends Reefiable, TARGET extends Reefiable = ORIGIN> =
